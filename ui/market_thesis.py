...
def _pair(v) -> tuple:
    """A CI/interval value → (lo, hi); (None, None) for anything not a 2+ seq."""
...
def _render_forecast(fc: dict | None) -> None:
    ...
    missing = rat.get("manifest_missing")
    missing = [str(x) for x in missing] if isinstance(missing, list) else []
    stale = rat.get("manifest_stale")
    stale = [str(x) for x in stale] if isinstance(stale, list) else []
    if missing or stale:
        st.caption(f"⚠ 缺事件:{', '.join(missing) or '—'}　舊事件:{', '.join(stale) or '—'}"
                   "　→ 故 degraded、不發警報。")

    events = rat.get("manifest_events")
        ...
        with st.expander("📅 巨觀事件 manifest(資料源 + 新鮮度)", expanded=False):
            ...
            for e in events:
                ...
                if e.get("value") is not None:
                    detail = f"{e.get('value')}" + (f" (Δ{e.get('delta_1d')})"
                                                    if e.get("delta_1d") is not None else "")
                elif e.get("last_rate"):
                    detail = f"{e.get('last_rate')} · 下次 {e.get('next_meeting_at')}"
                else:
                    detail = e.get("stale_reason") or "—"
                rows.append({
                    "事件": str(e.get("type", "?")),
                    "有資料": "✅" if e.get("present") else "❌",
                    "新鮮": "✅" if e.get("fresh") else "❌",
                    "來源": str(e.get("source_id", "—")),
                    "明細": str(detail),
                })
            if rows:
                st.dataframe(
...
def _render_validation(summ: dict | None) -> None:
    ...
    c = st.columns(4)
    _shared.metric_card(c[0], "驗證狀態", str(status))
    ...
    import pandas as pd
    rows = []
    for key, m in sorted(by_key.items(), key=lambda kv: str(kv[0])):
        if not isinstance(key, str) or not isinstance(m, dict):
            continue
        d, b, sc = (key.split("|") + ["?", "?", "?"])[:3]
        lo, hi = _pair(m.get("wilson90"))
        rows.append({
            "方向": d, "期程": b, "支撐": _SUPPORT_ZH.get(sc, sc),
            "命中/總數": f"{m.get('hits', 0)}/{m.get('counted_N', 0)}",
            "命中率": _pct(m.get("hit_rate"), 1),
            "CI90": f"[{_pct(lo, 1)}, {_pct(hi, 1)}]" if lo is not None else "—",
            "狀態": str(m.get("verdict", "?")),
        })
    if rows:
