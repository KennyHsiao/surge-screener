...
def test_refresh_core_sources_then_analytics_in_order() -> None:
    ...
    class FakeContinuationStrength:
        @staticmethod
        def run_report(*, features, output, analytics_dir=None, reports_dir=None, min_resolved=30):
            calls.append("continuation_strength")
            return {"status": "blocked", "reason": "missing daily bars", "resolved": 0, "min_resolved": 30}
    ...
    if result["playbook_validation"]["status"] != "blocked":
        raise AssertionError(result)
    if result["continuation_strength"]["status"] != "blocked":
        raise AssertionError(result)
...
def test_continuation_strength_error_does_not_fail_core_refresh() -> None:
    mod = _load_module()
    ...
    class FakePlaybookValidation:
        @staticmethod
        def run_validation(*, decisions, output, min_resolved=100):
            return {"status": "blocked", "reason": "missing decisions", "resolved": 0, "min_resolved": 100}

    class FailingContinuationStrength:
        @staticmethod
        def run_report(*, features, output, analytics_dir=None, reports_dir=None, min_resolved=30):
            raise RuntimeError("continuation validation crashed")
    ...
    if result["continuation_strength"]["status"] != "blocked":
        raise AssertionError(result)
    if "crashed" not in result["continuation_strength"]["reason"]:
        raise AssertionError(result)
