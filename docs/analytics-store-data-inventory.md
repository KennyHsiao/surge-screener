...
| `candidate_scores` | `reports/candidate_scores/YYYY-MM-DD.json` | one scored candidate per ticker/date/cohort | Retain complete LLM score history with explicit full-vs-bounded cohort provenance. Bounded top-N rows are selection-biased operational evidence, not full-universe validation; incomplete root runtime output is excluded. |
...
| `portfolio_positions` | optional `reports/reconciliation.json` | one underlying per IBKR reconciliation bucket | Position-aware analytics when explicitly configured; absence means `not_configured`, not “zero holdings” or producer failure. |
...
| `watchlist_sources` | `reports/watchlist.json`, `content/us_watchlist.txt` | one ticker per additive watchlist source | Explain why a ticker is visible. Scanner dates use refresh TTL; manual file dates are revision provenance only. |
| `source_observations` | presence and metadata of `reports/reconciliation.json`, `reports/watchlist.json` | one row per optional/scanner source observation | Preserve configured/missing/invalid state, observation date, reachability, and zero-row runs independently from ticker/position row counts. |
